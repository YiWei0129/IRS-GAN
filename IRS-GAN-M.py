...
def Jmodel_inputs(M, N, K,z_dim_G, z_dim_R):
    ...
    # lr_bias = tf.placeholder(tf.float32, name='lr')
...
def discriminator(channel, M, N, name, reuse=False):
...
def discriminator_Multi(channel_set, M, N, K, reuse=False):
    ...
    if mode == 1:
        with tf.variable_scope('generator', reuse=not is_train):
            G0 = tf.layers.dense(z, M * N * 5, tf.nn.tanh)
            G1 = tf.layers.dense(G0, M * N * 10, tf.nn.tanh)
            G2 = tf.layers.dense(G1, M * N * 5, tf.nn.tanh)
            Gout = tf.layers.dense(G2, M * N * 2, name='Gout')
    else:
        if mode == 2:
            with tf.variable_scope('generator', reuse=not is_train):
                fc0_temp = tf.layers.dense(z, M * N * 4, tf.nn.tanh)
                fc0 = tf.reshape(fc0_temp, (-1, M * 2, N * 2, 1))
                conv1_g = tf.layers.conv2d(inputs=fc0, filters=256, kernel_size=5, padding='same')
                # conv1_g_bn = tf.layers.batch_normalization(conv1_g, training=training)
                conv1_g = tf.nn.tanh(conv1_g)
                conv2_g = tf.layers.conv2d(inputs=conv1_g, filters=128, kernel_size=3, padding='same')
                conv2_g = tf.nn.tanh(conv2_g)
                conv3_g = tf.layers.conv2d(inputs=conv2_g, filters=64, kernel_size=3, padding='same')
                conv3_g = tf.nn.tanh(conv3_g)
                conv4_g = tf.layers.conv2d(inputs=conv3_g, filters=1, kernel_size=3, padding='same')
                fc1_temp = tf.reshape(conv4_g, (-1, M * N * 4))
                Gout = tf.layers.dense(fc1_temp, M * N * 2, tf.nn.tanh, name='Gout')
        else:  # (MODE = 3)
            with tf.variable_scope('generator', reuse=not is_train):
                if BN == 0:
                    fc0 = tf.layers.dense(z, M * N * 2, tf.nn.tanh)
                    fc1 = tf.layers.dense(fc0, M * N * 4, tf.nn.tanh)
                    # learn G
                    G_g = tf.layers.dense(fc1, M * N * 4, tf.nn.tanh)
                    G_g_2D = tf.reshape(G_g, (-1, M * 2, N * 2, 1))
                    ...
                    G_conv4_g = tf.layers.conv2d(inputs=G_conv3_g, filters=1, kernel_size=3, padding='same')
                    G_fc1 = tf.reshape(G_conv4_g, (-1, M * N * 4))
                    G_out_temp = tf.layers.dense(G_fc1, M * N * 2)
                    G_out_temp1 = tf.complex(G_out_temp[:, 0:M * N], G_out_temp[:, M * N:M * N * 2])
                    G_out = tf.reshape(G_out_temp1, (-1, M, N))

                    # learn R
                    R_fc1_g = tf.layers.dense(fc1, M * N * 2, tf.nn.tanh)
                    R_fc2_g = tf.layers.dense(R_fc1_g, M * N, tf.nn.tanh)
                    R_out_temp = tf.layers.dense(R_fc2_g, N * 2)
                    R_out = tf.complex(R_out_temp[:, 0: N], G_out_temp[:, N:N * 2])

                    Gout_temp = tf.matmul(G_out, tf.matrix_diag(R_out))
                    Gout = tf.concat(
                        [tf.reshape(tf.real(Gout_temp), (-1, M * N)), tf.reshape(tf.imag(Gout_temp), (-1, M * N))], 1,
                        name='Gout')
                else:
                    with tf.variable_scope('generator_FN', reuse=not is_train):
                        fc0 = tf.layers.dense(z, M * N * 2, tf.nn.tanh)
                        fc1 = tf.layers.dense(fc0, M * N * 4, tf.nn.tanh)
                        # learn G
                    with tf.variable_scope('generator_GN', reuse=not is_train):
                        ...
                        G_conv4_g = tf.layers.conv2d(inputs=G_BN3_g, filters=1, kernel_size=3, padding='same')
                        G_fc1 = tf.reshape(G_conv4_g, (-1, M * N * 4))
                        G_out_temp = tf.layers.dense(G_fc1, M * N * 2)
                        G_out_temp1 = tf.complex(G_out_temp[:, 0:M * N], G_out_temp[:, M * N:M * N * 2])
                        G_out = tf.reshape(G_out_temp1, (-1, M, N))
                    ...
                    with tf.variable_scope('generator_RN', reuse=not is_train):
                        ...
                        R_out_temp = tf.layers.dense(R_fc2_g, N * 2)
                        R_out = tf.complex(R_out_temp[:, 0: N], G_out_temp[:, N:N * 2])
                    with tf.variable_scope('generator_Out', reuse=not is_train):
                        Gout_temp = tf.matmul(G_out, tf.matrix_diag(R_out))
                        Gout = tf.concat(
                            [tf.reshape(tf.real(Gout_temp), (-1, M * N)), tf.reshape(tf.imag(Gout_temp), (-1, M * N))],
                            1,
                            name='Gout')
    return Gout
...
def JWGAN_GP_model_loss(H_mean_set, channel_real, input_z_G, input_z_R, z_R_dim, M, N, K, BiasVar_G,BiasVar_R_Set, batch_size=50,LAMBDA=10, Mu=10):
...
def model_opt_JWGAN_GP(d_loss_set, Jg_loss, learning_rate, lr_b_G, lr_b_R_set, K):
    ...
    with tf.control_dependencies(update_ops):
            ...
            for k in range(K):
                g_train_opt_Bias_R = tf.train.AdamOptimizer(lr_b_R_set[k], beta1=0.5).minimize(Jg_loss, var_list=g_vars_Bias_R_set[k])
            ...
            for k in range(K):
                d_train_opt = tf.train.AdamOptimizer(learning_rate, beta1=0.5).minimize(d_loss_set[k], var_list=d_vars_set[k])
                d_train_opt_set.append(d_train_opt)
    return d_train_opt_set, Jg_train_opt, g_vars, d_vars_set


def Jtrain(H_mean_set, H_mean_mat_set, singluar_real_set, BiasVar_G,BiasVar_R_Set, RandomVar_G,RandomVar_R, epoch_count, plot_every, batch_size, z_dim_G, z_dim_R, M, N, load_path, lr_d, lr_g, lr_bias_G, lr_bias_R_set,
    ...
    d_loss_set, Jg_loss, d_model_fake_set, d_model_real_set, g_model_set, d_logits_real_set, d_logits_fake_set = JWGAN_GP_model_loss(H_mean_set, input_real_set, input_z_G, input_z_R, z_dim_R, M, N, K, BiasVar_G,BiasVar_R_Set, batch_size,
                       LAMBDA, Mu)
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch_i in range(epoch_count + 1):
            ...
            if steps == 1:
                    ...
                    for k in range(K):
                        train_loss_d = d_loss_set[k].eval({input_real_set: batch_channel, input_z_G: batch_z_G, input_z_R: batch_z_R})
                ...
                print("save model JIRS_GAN_model_step_{}".format(epoch_i), file=mylog)
            ...
            for d_idx in range(iterIn_d):
                ...
                if (d_idx+1) % 200 == 0:
                    print("Epoch {}/{}...".format(epoch_i, epoch_count), "d_iter {}/{}...".format(d_idx + 1, iterIn_d))
                    for k in range(K):
                        train_loss_d = d_loss_set[k].eval({input_real_set: batch_channel, input_z_G: batch_z_G, input_z_R: batch_z_R})
                        train_d_model_real = d_model_real_set[k].eval(
                        {input_real_set: batch_channel, input_z_G: batch_z_G, input_z_R: batch_z_R})
...
for k in range(K):
    ...
    singluar_real_set.append(singluar_real)
    H_mean_set.append(H_mean_k)
    H_mean_mat_set.append(H_mean_k_mat)

# train

singular_test_set, singular_var_test_set =  Jtrain(H_mean_set, H_mean_mat_set, singluar_real_set, BiasVar_G, BiasVar_R_set, RandomVar_G,RandomVar_R, epochs, plot_every, batch_size, z_dim_G, z_dim_R, M, N,load_path, lr_d, lr_g, lr_bias_G, lr_bias_R_set, iterIn_d,iterIn_g, H_SET_K_MAT,mean_max_set,K,Mu,
     WGAN_GP,LAMBDA)
